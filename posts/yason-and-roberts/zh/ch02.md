...
[English](../en/ch02.md) | [简体中文](./ch02.md)
...
graph TB
    subgraph "角色模糊 = 灾难"
        M[Kai + Rex<br>模糊角色] --> C[任务冲突]
        M --> C[重复劳动]
        M --> C[仲裁成本飙升]
    end

    subgraph "角色清晰 = 高效"
        M[Kai → 全栈开发<br>Rex → 测试与 SRE<br>Max → Leader / PMO] --> C[各司其职]
        M --> C5[零冲突]
        M --> C6[产出最大化]
    end
...
### Rex：QA & SRE — 质量与稳定的守护者

Rex 是团队里最"安静"的那个。他的工作不像 Kai 那样快节奏——测试和运维是个需要耐心和精度的活儿，一个回归测试跑几小时是常事，线上故障排查更是要稳扎稳打。

**Rex 的专属工具链：**

- 测试自动化框架 — 单元测试、集成测试、E2E 测试
- 监控与告警系统 — Prometheus、Grafana、日志分析
- CI/CD 流水线 — 构建、部署、回滚

Rex 的风格和 Kai 完全不同。Kai 喜欢给 Yason 发问题让 Yason 决策，Rex 则倾向于自己先跑一遍测试、收集数据，再给 Yason 一个对比表让 Yason 选。

这其实反映了两个领域的本质差异：

- **开发**：改得快、重构容易，适合"边问边做"
- **测试与运维**：改一次影响大、排查周期长，适合"先测再上线"

Yason 没有刻意设计这种差异，而是 AI Agent 在工作过程中自动适配了领域特性。

### Max：Leader / PMO — 团队的指挥中枢

Max 是团队的"超级战士"——Leader、PMO、运营、内容创作、数据分析、团队协调，样样都要干。如果说 Kai 是技术大脑，Rex 是质量守门人，那 Max 就是把一切串起来的那个人。
...
graph LR
    subgraph "罗伯特三剑客"
        K[Kai<br>Full-Stack Dev<br> 全栈开发<br>风格：边问边做]
        R[Rex<br>QA & SRE<br> 测试与运维<br>风格：先测再上线]
        M[Max<br>Leader / PMO<br> 项目管理与协调<br>风格：自适应降级]
    end
...
Kai 说："我要做架构重构，连续跑 8 小时。"Rex 说："我在跑回归测试套件，需要大量算力，也是 8 小时。"
...
graph LR
    subgraph "阶段一：单人模式"
        S[Yason +  个 Agent] --> S[探索期<br>手动写提示词<br>手动 review]
    end

    subgraph "阶段二：双人模式"
        S[Yason +  个 Agent] --> S[扩展期<br>角色分工出现<br>角色冲突也出现]
    end

    subgraph "阶段三：团队模式"
        S5[Yason +  个 Agent<br>+ 子 Agent] --> S6[成熟期<br>Yason 退居管理岗<br>Agent 自主协作]
    end
