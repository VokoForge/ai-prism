...
> 「skill」这个词在 2025 年底一夜之间从"Lisp 的 closure"变成了"Agent 生态的 markdown 文件夹"。到 2026 H1，GitHub 上 star 过千、跟 Agent 强相关的 skill 仓库已经有 200+。但大多数是 demo，一周不再更新。真正配得上"star"的，我选了这 3 个（另外 3 个简评附后）。
...
quadrantChart
    ...
    quadrant-3 平庸区
    quadrant-4 实用工具
    agent-skills: [0.7, 0.9]
    writing-agent: [0.9, 0.8]
    wewrite: [0.6, 0.85]
    vibe-blog: [0.8, 0.7]
    claude-code-best-practice: [0.3, 0.95]
    learn-claude-code: [0.5, 0.75]
```
...
**不足：** 它是索引不是实现，真用要 clone 里面推荐的 sub-repo。没有中文版。
...
**不足：** 默认用 Claude Sonnet，便宜模型支持差。配图 Agent 需要科学上网。禁词黑名单是中文圈的，写英文要自己改。
...
**不足：** 默认用 Qwen（需要 DashScope API key）。LangGraph 部分需要 Python 3.12 + uv，老项目迁移成本中等。
