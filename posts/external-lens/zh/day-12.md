...
**之前：AI 写的文章千篇一律 → 现在：用风格指纹约束生成 → 这意味着：每篇文章都有"人味"。**

说白了，写作 DNA 就像 DNA 亲子鉴定——不需要测全部基因，6-12 个位点就够了。
...
| 策略 | 做法 | 优点 | 缺点 |
|------|------|------|------|
| A: 注入软提示 | 指纹摘要加进 system prompt | 零代码 | 长输出会漂移 |
| B: 后处理改写 | 生成后测指纹，选最接近的候选 | 强制约束 | 推理成本翻倍 |
| C: 硬规则过滤 | 拒绝违反硬阈值的生成 | 确定性 | 长文必失败 |
