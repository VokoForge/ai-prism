...
**之前：10M 向量查询 3.2s → 现在：0.04s → 这意味着：差了 80 倍，RAG pipeline 从"能用"变成"丝滑"。**
...
```mermaid
flowchart LR
    LLM[LLM Tool Call] -->|WebSocket| FE[前端 React 组件]
    FE -->|用户操作| LLM
    FE -->|渲染| UI[可交互 UI]
...
**之前：Agent 前端每个项目从零写 → 现在：CopilotKit 组件开箱即用 → 这意味着：Agent 的差异化不在"能做什么"，在"怎么让人参与"。**

2024 年大家卷 Agent 后端编排（LangGraph、CrewAI），2026 年卷的是 Agent 前端体验。CopilotKit 的爆发说明了一切。
...
**之前：RAG = 检索 + 拼接 → 现在：RAG = 检索 → 理解 → 重构 → 这意味着：知识 Agent 终于不是"缝合怪"了。**

open-notebook 的 DAG 模式可能是知识 Agent 的标准架构。
...
**之前：定价 2999，3 个月卖 200 台 → 现在：如果定价 599，可能 3 个月卖 2000 台 → 这意味着：AI 硬件的护城河不在模型，在供应链和渠道。**
...
AI 硬件的问题更根本：技术团队做硬件，至少要配一个做过消费电子的合伙人。硬件有库存压力、供应链风险、退货率。一个定价错误就能把公司拖死。
